...
class ResNet_s(nn.Module):
    ...
    def __init__(self, block, num_blocks, num_classes=10, use_norm=False, head_tail_ratio = 0.3, transfer_strength = 1.0, phase_train=True, epoch_thresh=0):
        ...
        self.conv1 = nn.Conv2d(1, 14, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(14)
        
        self.layer1 = self._make_layer(block, 14, num_blocks[0], stride=1)
        self.layer2 = self._make_layer(block, 28, num_blocks[1], stride=2)
        self.layer3 = self._make_layer(block, 56, num_blocks[2], stride=2)
        if use_norm:
            self.linear = NormedLinear(56, num_classes)
        else:
        ...
        if self.phase_train:
           ...
           self.RSG = RSG(n_center = 15, feature_maps_shape = [28, 14, 14], num_classes=num_classes, contrastive_module_dim = 256, head_class_lists = self.head_lists, transfer_strength = transfer_strength, epoch_thresh = epoch_thresh)
